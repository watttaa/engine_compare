...
import { SpriteChannel } from './sprite-channel.js';
...
export class WebGPURenderer {
    constructor() {
        ...
        this._blurBGL = null;
        this._blurUbuf = null;
        this._spriteChannel = null;
        this._instanceDrawCalls = 0;
    }
    static async create(canvas) {
        ...
        // 尝试请求超过默认 16 的采样纹理槽（D3D11 SM5 支持 128/阶段，Dawn 实测本机支持 48）。
        // 32 张模型图集 > 默认 16 槽导致实例批碎片化（2080 人 = 390 批）；请求失败自动回退默认。
        // 注意：同一 adapter 只能创建一个 device，故不能先 probe——直接请求，失败再走默认。
        let device;
        try {
            device = await adapter.requestDevice({ requiredFeatures: features, requiredLimits: { maxSampledTexturesPerShaderStage: 48 } });
        } catch (e) {
            console.warn('[webgpu] requiredLimits 48 请求失败，回退默认 limits:', String(e && e.message).slice(0, 120));
            device = await adapter.requestDevice({ requiredFeatures: features });
        }
        console.log('[webgpu] maxSampledTexturesPerShaderStage =', device.limits.maxSampledTexturesPerShaderStage);
        ...
        if (r._multiTex) {
            ...
            r._texSlots = Math.max(1, Math.min(64, limit));
    ...
    begin(clear = { r: 0.05, g: 0.07, b: 0.09, a: 1 }, target = null) {
        ...
        this._batch.reset();
        this._instanceDrawCalls = 0;
        if (this._spriteChannel)
            this._spriteChannel.batchCount = 0;
    }
    end() {
        ...
        const drawCalls = this._batch.drawCalls + this._instanceDrawCalls;
        this._lastBatchCount = this._batch.drawCalls;
        this._lastInstanceBatchCount = this._spriteChannel ? this._spriteChannel.batchCount : 0;
    ...
    getSpriteChannel() {
        if (!this._spriteChannel)
            this._spriteChannel = new SpriteChannel(this);
        return this._spriteChannel;
    }
    setBlendMode(mode) {

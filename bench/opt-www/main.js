...
(function () {
  ...
  MCBenchMain.prototype._onAdded = function () {
    ...
    hud.innerHTML =
      ...
      '<input id="mcnt" type="number" value="10" step="10">' +
    ...
    var _hudFrameBuf = [];   // 最近帧时长（ms），算实时 p95 —— 压测采样中给 HUD 出实时指标
    var _lvlStartTs = 0;     // 当前档位 onLevel start 时刻，算采样进度
    var capacityStatus = '';
    var _origTick = runner.tick.bind(runner);
    runner.tick = function (ts) {
      _origTick(ts);
      if (_lastTs2) {
        var _ft = Math.min(ts - _lastTs2, 100);
        _fpsBuf = _fpsBuf * 0.9 + _ft * 0.1;
        _hudFrameBuf.push(_ft);
        if (_hudFrameBuf.length > 240) _hudFrameBuf.shift();
      }
      _lastTs2 = ts;
      if (ts - _lastLive > 400) {
        _lastLive = ts;
        ...
        if (capacityRunning) {
          // 压测采样中：状态 + 实时指标（此前 capacityRunning 时 HUD 冻结，采样 10-18s 无任何输出）
          var _buf = _hudFrameBuf.slice().sort(function (a, b) { return a - b; });
          var _p95 = _buf.length ? _buf[Math.min(_buf.length - 1, Math.floor(_buf.length * 0.95))] : 0;
          var _el = _lvlStartTs ? ((performance.now() - _lvlStartTs) / 1000).toFixed(0) : '-';
          liveEl.textContent = capacityStatus +
            '\n[实时] fps: ' + (1000 / _fpsBuf).toFixed(1) +
            '  帧p95: ' + _p95.toFixed(1) + 'ms  已进行: ' + _el + 's';
        } else {
          liveEl.textContent =
            '后端: ' + backend + '  地图: ' + mapSt + '\n' +
    ...
    // 自动双后端：跨页状态机在 sim-core/mc-compare.js（引擎无关，Laya/Cocos 臂复用）
    var mcCompare = new MCCompare({
      engine: 'egret-selfdev-5.4.1',
      webglPage: '/egret-mc/index.html',
      webgpuPage: '/opt-www/index-opt.html',
      webgpuQuery: 'channel=instanced',
      onProgress: function (msg) { liveEl.textContent = msg; },
      onDone: function (result) {
        lastResult = result;
        copyBtn.disabled = false;
        var webgl = result.webgl, webgpu = result.webgpu;
        var lift = webgl.maxStableCount > 0 ? (webgpu.maxStableCount / webgl.maxStableCount).toFixed(2) : '?';
      ...
      runner.capacityRamp({
        ...
        onLevel: function (info) {
          if (info.phase === 'start') {
            ...
            // 稳定判定 = p95≤20 且 p99≤35（bench-runner capacityRamp）；不稳定时列出超标项
            function stableWhy(j) {
              var r = [];
    ...
    compareBtn.addEventListener('click', function () { if (!capacityRunning) mcCompare.start(); });

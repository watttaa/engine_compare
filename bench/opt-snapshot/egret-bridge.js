...
export class WebGPUSystemRenderer {
    ...
    _drawBatchedSpriteLayer(layer, buffer) {
        ...
        this.r._batch.flush();
        channel.beginFrame();
        // 逐 MC 取世界矩阵：MovieClip 的 $matrix 不反映帧动画位置（frame offset 走
        // $getConcatenatedMatrix），故不能用本地矩阵组合。引擎已按 $GlobalFrameID 把 concat
        // 缓存到 $concatenatedMatrix，无参调用即命中缓存，无需复用矩阵。
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (!dp.$visible || !dp.renderVisible || dp.$alpha <= 0 || dp.$maskedObject || dp.$filters || dp.filters || dp.$cacheAsBitmap)
                return false;
            // P2R：直接镜像 MovieClip 引擎状态（$texture/offsetPoint 由 constructFrame 每帧维护），
            // 跳过 $getRenderNode/RenderNode/drawData 整条重路径。
            // $texture 由引擎构造帧维护、非空即有效，省去逐 MC 的 $isTextureValid() 校验。
            const frameTex = dp.$texture;
            if (!frameTex || !frameTex.$bitmapData) { channel.skipSprite(dp); continue; }
            const gpuTex = this.getTexture(frameTex.$bitmapData);
            if (!gpuTex)
                return false;
            ...
            if (!channel.mirrorSprite(dp, gpuTex, frameTex, op ? op.x : 0, op ? op.y : 0, cm, root, alpha))
                return false;
        }
        channel.flush();

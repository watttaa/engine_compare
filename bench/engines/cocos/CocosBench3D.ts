...
export class CocosBench3D extends Component {
    ...
    private backend(): string {
        ...
        if (dev && dev.gfxAPI !== undefined) {
            // gfxAPI: WEBGL=1, WEBGL2=2, WEBGPU=6（cocos 3.8 gfx 定义）
            return dev.gfxAPI === 6 ? 'webgpu' : 'webgl';
        }
...
class Cocos3DAdapter {
    ...
    private workPos: Float32Array[] = [];   // 预分配工作缓冲（避免每帧 new）
    ...
    private makeFish(f: any) {
        ...
        if (!data || !mat) {
            ...
            this.workPos.push(new Float32Array(0));
        ...
        this.workPos.push(new Float32Array(vc * 3));  // 预分配，step() 复用
    ...
    step(ts: number) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            const out = this.workPos[i];   // 复用预分配缓冲，无 GC
            const species = f.species % 5;

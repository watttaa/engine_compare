...
(function (global) {
    ...
    BenchRunner.prototype.fixedRun = function (opts) {
        ...
        function startStats() {
            ...
            if (self.stats.captureGpuInfo)
                self.stats.captureGpuInfo();
    ...
    BenchRunner.prototype.rampRun = function (opts) {
        ...
        if (this.stats.captureGpuInfo)
            this.stats.captureGpuInfo();
    ...
    BenchRunner.prototype.tick = function (ts) {
        ...
        if (ts - r.lastStepTs >= r.stepMs) {
            r.lastStepTs = ts;
            if (cur + r.stepCount <= r.maxCount)
                this.adapter.setCount(cur + r.stepCount);
        }
    };
    /**
    ...
    BenchRunner.exportJSON = function (json) {
        ...
        function writeHandle() {
            BenchRunner.saveDirHandle.getFileHandle(name, { create: true }).then(function (fh) {
                ...
                if (BenchRunner.onSaved)
                    BenchRunner.onSaved(name);
            }).catch(function (e) {
        ...
        function copyToClipboard() {
            var done = function () { if (BenchRunner.onCopied)
                BenchRunner.onCopied(text); };
            function fallback() {

...
(function (global) {
    ...
    function initPathfinder(mapBytes) {
        ...
        var PF = global.PF;
        if (!PF) {
            console.warn('[MCSim] pathfinding.js 未加载，寻路降级为随机游走');
            return;
        }
        // 构造 PF.Grid
        ...
        if (bits < cells)
            console.info('[MCSim] 阻挡位图末尾缺少 ' + (cells - bits) + ' 格，已按游戏阻挡规则处理');
    ...
    function findPath(sx, sy, ex, ey) {
        if (!_mapLoaded || !_pfFinder)
            return null;
        var g = _pfGrid.clone();
        var path = _pfFinder.findPath(sx, sy, ex, ey, g);
        if (path && path.length > 1)
            path = PF.Util.smoothenPath(g, path);
    ...
    function randomNearbyWalkable(cx, cy) {
        ...
        for (var t = 0; t < 30; t++) {
            ...
            if (_mapLoaded ? _pfGrid.isWalkableAt(nx, ny) : true)
                return { x: nx, y: ny };
        }
        return { x: cx, y: cy };
    }
    ...
    MCSim.prototype.add = function (n) {
        for (var i = 0; i < n; i++)
            this.list.push(this._spawn(this.list.length));
    };
    ...
    MCSim.prototype.update = function (dtMs) {
        ...
        for (var i = 0; i < list.length; i++) {
            var agent = list[i];
            if (!agent || !agent.path || agent.pathIdx >= agent.path.length)
                continue;

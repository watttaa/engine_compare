...
function loadChar(charId) {
    ...
    ]).then(layers => ({ data0: layers.slice(0, 3), data2: layers.slice(3, 6) }));
    return _resCache[key];
}
...
class LayaMCAdapter {
    ...
    setCount(n) {
        ...
        if (n > cur) {
            ...
            for (let i = cur; i < n; i++) {
                ...
                const node = { cont: cont, players: [], loaded: false, dir: entry.dir, resDir: 0, res: null };
                this.nodes[i] = node;
                this._pendingLoads++;
                loadChar(entry.charId).then((res) => {
                    self._pendingLoads--;
                    if (!self.sim || self._loadEpoch !== loadEpoch || i >= self.sim.list.length || self.nodes[i] !== node)
                        return;
                    node.res = res;
            ...
            for (let j = cur - 1; j >= n; j--) {
                if (this.nodes[j] && this.nodes[j].cont)
                    this.nodes[j].cont.removeSelf();
                this.nodes.pop();
            }
        }
    }
    step(ts) {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            if (!nd || !nd.loaded || !list[i])
                continue;
            nd.cont.pos(list[i].x, list[i].y);
            for (let mi = 0; mi < nd.players.length; mi++)
                nd.players[mi].tick(dt);
        }
    }
    readDrawCalls() {
        const L = G.Laya;
    ...
    nodeCount() {
        return this.nodes.filter((n) => n && n.loaded).length;
    }
    isReady() {
...
class LayaBench_MC {
    static start() {
        ...
        xhrBin(MAP_ROOT + '1001.map').then((ab) => {
            G.MCSim.initPathfinder(new Uint8Array(ab));
        }).catch((e) => console.warn('[mc-laya] 地图位图加载失败（寻路降级随机游走）:', e));
        ...
        runner.tick = (ts) => {
            ...
            if (_lastTs2) {
                ...
                if (LayaBench_MC._hudFrameBuf.length > 240)
                    LayaBench_MC._hudFrameBuf.shift();
            }
            _lastTs2 = ts;
            if (adapter._resolvedBackend === undefined) {
                ...
                for (const c of canvases) {
                    if (!main || (c.width * c.height) > (main.width * main.height))
                        main = c;
                }
    ...
    // ---------------- HUD ----------------
    static buildHud(Laya, runner, adapter, requested) {
        ...
        const liveEl = hud.querySelector('#lmlive');
        LayaBench_MC.liveEl = liveEl;
        LayaBench_MC.$c = hud.querySelector('#lmcnt');
        const copyBtn = hud.querySelector('#lmcopy');
        const stopBtn = hud.querySelector('#lmstop');
        const compareBtn = hud.querySelector('#lmcompare');
        function setCapacityRunning(running) {
            LayaBench_MC.capacityRunning = running;
            compareBtn.disabled = running;
            stopBtn.disabled = !running;
            hud.querySelector('#lmfixed').disabled = running;
            if (LayaBench_MC.$c)
                LayaBench_MC.$c.disabled = running;
        }
        ...
        const mcCompare = new G.MCCompare({
            ...
            onProgress: (msg) => { liveEl.textContent = msg; },
            onDone: (result) => {
        ...
        compareBtn.addEventListener('click', () => { if (!LayaBench_MC.capacityRunning)
            mcCompare.start(); });
        stopBtn.addEventListener('click', () => { runner.stopCapacityRamp(); });
        window.__lmSetCapacityRunning = setCapacityRunning;
        window.__lmMcCompare = mcCompare;
    }
    static runFixed(runner, adapter, requested) {
    ...
    static runCapacity(runner, adapter, requested) {
        const v = 'M2';
        const capacityCounts = new URLSearchParams(location.search).get('capacityCounts');
        const counts = capacityCounts ? capacityCounts.split(',').map(Number).filter((n) => n > 0) : null;
        const setRunning = window.__lmSetCapacityRunning;
        setRunning(true);
        ...
        runner.capacityRamp({
            ...
            onLevel: (info) => {
                if (info.phase === 'start') {
                    if (LayaBench_MC.$c)
                        LayaBench_MC.$c.value = String(info.count);
                    ...
                    const stableWhy = (j) => {
                        const r = [];
                        if (!(j.p95 <= 20))
                            r.push('p95 ' + j.p95 + '>20ms');
                        if (!(j.p99 <= 35))
                            r.push('p99 ' + j.p99 + '>35ms');
                        return r.join(' · ');
                    };
            ...
            onDone: (summary) => {
                ...
                const json = {
                    ...
                    results: summary.results.map((r) => ({ count: r.count, stable: r.stable, result: r.json }))
                };
                LayaBench_MC.lastResult = json;
                const copyBtn = document.querySelector('#lmcopy');
                if (copyBtn)
                    copyBtn.disabled = false;
                LayaBench_MC.liveEl.textContent = summary.cancelled
                    ...
                    : '承载力压测完成\n最大稳定承载力: ' + summary.cap + ' 角色' +
                        ...
                        '\n已完成档位: ' + summary.results.length;
                G.BenchRunner.exportJSON(json);
                const mcCompare = window.__lmMcCompare;
                if (mcCompare)
                    mcCompare.continueAfterArm(json);
            }
        });
    }
    static updateLive(adapter, fpsBuf) {
        ...
        if (LayaBench_MC.capacityRunning) {
            const buf = LayaBench_MC._hudFrameBuf.slice().sort((a, b) => a - b);
...
exports.LayaBench_MC = LayaBench_MC;
LayaBench_MC.liveEl = null;
LayaBench_MC.$c = null;
LayaBench_MC.capacityRunning = false;
LayaBench_MC.lastResult = null;
LayaBench_MC.capacityStatus = '';
LayaBench_MC._lvlStartTs = 0;
LayaBench_MC._hudFrameBuf = [];
LayaBench_MC._mapContHolder = null;

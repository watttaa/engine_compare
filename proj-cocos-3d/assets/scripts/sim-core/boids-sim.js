...
(function (global) {
  ...
  var FISH_SPECIES = 5;
  var DEG = Math.PI / 180;
  // 鱼种视觉缩放补偿（fish_1 长鱼 / fish_2 鲨鱼 / fish_3 小鱼，species = idx % 5）
  var SPECIES_SCALE = [0.45, 0.55, 1.0, 0.45, 0.55];
  ...
  /** 迁移 cpp setType 的角度区间，但收窄为"从右往左"：
   *  鱼从右边缘出生，朝左游（角度在 90°~270° 之间的左向区间），
   *  保留 cpp 的每帧角度单调摆动 + 往返的游动手感。 */
  function angleTypeOf(angType) {
    var r = Math.random;
    switch (angType) {
      case 0: return [160 + r() * 20, 200 + r() * 20];   // 左
  ...
  BoidsSim.prototype._spawn = function (species) {
    ...
    return {
      species: species,
      x: x, y: y,
      angle: initAngle * DEG,          // 当前角度（弧度）
      minAngle: minA * DEG,
      maxAngle: maxA * DEG,
      angleAdd: Math.random() < 0.5,   // cpp 的 angleAdd：true 则角度 ++，false 则 --
      speed: 2 + Math.random() * 2,    // 2~4 px/帧
      scale: (0.6 + Math.random() * 0.8) * SPECIES_SCALE[species % FISH_SPECIES]
    };
  };

  /** 每帧推进：迁移 cpp frameFun/moveForword */
  BoidsSim.prototype.update = function (dtMs) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      // cpp frameFun：角度在 [min,max] 间每帧单调增减
      if (f.angleAdd) f.angle += 0.5 * DEG * step;
      else f.angle -= 0.5 * DEG * step;
      if (f.angle > f.maxAngle) { f.angle = f.maxAngle; f.angleAdd = false; }
      else if (f.angle < f.minAngle) { f.angle = f.minAngle; f.angleAdd = true; }

      // cpp moveForword：沿角度直线前进
      f.x += Math.cos(f.angle) * f.speed * step;
      f.y += Math.sin(f.angle) * f.speed * step;

      // cpp inScreen：出大包围盒 → 重生

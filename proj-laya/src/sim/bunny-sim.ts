...
(function (global) {
  ...
  /** 添加 n 只兔子。initX 复刻原版：(count % 2) * 800 */
  ...
  BunnySim.prototype.update = function () {
    ...
    for (var i = 0, len = list.length; i < len; i++) {
      ...
      if (bny.x > b.right) {
        bny.speedX *= -1; bny.x = b.right;
      } else if (bny.x < b.left) {
        bny.speedX *= -1; bny.x = b.left;
      }
      if (bny.y > b.bottom) {
        bny.speedY *= -0.85; bny.y = b.bottom;
        if (Math.random() > 0.5) bny.speedY -= Math.random() * 6;
      } else if (bny.y < b.top) {
        bny.speedY = 0; bny.y = b.top;
      }
    }
  };

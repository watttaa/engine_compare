...
(function (global) {
  ...
  BenchStats.prototype.tick = function (ts) {
    ...
    if (this._phase === 'prewarm') {
      ...
      if (this._warmFrames.length > 40) this._warmFrames.shift();
      var warmOk = false;
      if (this._warmFrames.length >= 30) {
        ...
        var wavg = 0;
        for (var wi = 0; wi < ws.length; wi++) wavg += ws[wi];
        wavg /= ws.length;
        warmOk = wp95 <= 20 && wavg <= 18.2;
      }
      if (warmOk && !this._calmSince) this._calmSince = this._elapsed;
      if (!warmOk) this._calmSince = 0;
      this._elapsed += dt;
      ...
      if (this._elapsed >= this._phaseEnd && (converged || this._elapsed >= this._maxWarm) || this._elapsed >= this._maxWarm) {
        ...
        if (this._readDrawCalls) this._readDrawCalls();
      }
      return;
    }
  ...
  BenchStats.prototype.toJSON = function () {
    ...
    var sum = 0, over = 0, i;
    for (i = 0; i < s.length; i++) { sum += s[i]; if (s[i] > 16.7) over++; }
    ...
    if (s.length) {
      ...
      var tailSum = 0;
      for (i = s.length - tailN; i < s.length; i++) tailSum += s[i];
      p1Low = tailSum / tailN;
      var variance = 0;
      for (i = 0; i < s.length; i++) { var d = s[i] - avg; variance += d * d; }
